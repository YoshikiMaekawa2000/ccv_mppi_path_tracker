...
class RecordState:
    def __init__(self):
        ...
        self.sub_zmp_y = rospy.Subscriber('/ccv_mppi_path_tracker/zmp_y', std_msgs.msg.Float64, self.zmp_y_callback)
        self.sub_drive_accel = rospy.Subscriber('/ccv_mppi_path_tracker/drive_accel', std_msgs.msg.Float64, self.drive_accel_callback)
...
##################callback関数##################
    ...
    def drive_accel_callback(self, msg):
        self.drive_accel = msg.data
        self.drive_accel_flag = True
##############################################
    ...
    def __call__(self):
        ...
        self.csv_writer.writerow(["time", "zmp_y", "roll", "drive_accel"])
        start = rospy.get_time()
        last_v = 0
        last_time = rospy.get_time()
        while not rospy.is_shutdown():
            # if(tf_x < 15):
            if(self.joint_state_flag and self.zmp_y_flag):
                
                # self.csv_writer.writerow([rospy.get_time() - start, self.zmp_y, roll])
                ...
                roll = self.joint_state.position[2]
                self.csv_writer.writerow([rospy.get_time() - start, self.zmp_y, roll, self.drive_accel])
                rate.sleep()
            # else:
                # pass
